...
function setupTouchSwipe() {
    const containers = [hourlyForecastElement, dailyForecastElement];
    ...
    containers.forEach(container => {
        ...
        let lastX;
        let timestamp;
        const friction = 0.97; // 마찰 계수 (높을수록 더 오래 스크롤)
        ...
        container.addEventListener('touchmove', (e) => {
            ...
            // 현재 속도 계산
            const now = Date.now();
            const dt = now - timestamp || 1; // 0으로 나누기 방지
            velocity = (lastX - x) / dt * 100; // 속도 계수 조정 
        ...
        container.addEventListener('touchend', () => {
            ...
            // 관성 스크롤 시작
            startMomentumScroll(container, velocity);
        }, { passive: true });
...
function startMomentumScroll(element, initialVelocity) {
    ...
    if (Math.abs(initialVelocity) > 0.5) {
        ...
        // 속도가 충분하지 않으면 가장 가까운 아이템으로 스냅
        snapToClosestItem(element);
    }
}

// 가장 가까운 아이템으로 스냅
function snapToClosestItem(element) {
    const items = element.querySelectorAll('.hourly-item, .daily-item');
    if (items.length === 0) return;
    
    const containerWidth = element.offsetWidth;
    const scrollLeft = element.scrollLeft;
    
    // 현재 스크롤 위치에서 가장 가까운 아이템 찾기
